...
/**********************************************************************
  ...
  2: Construction is simplified ... all you need are wood-working drills.

  3: Extremely low cost as very few parts.

  Disadvantages:
  1: long lines exhibit a slight curvatature. This curvature can be masked by 
  inserting extra plotting points.
...
#define PEN 3
#define OFFSET1 50               //motor1 offset along x_axis
#define OFFSET2 110                 //motor2 offset along x_axis
#define YAXIS 200                  //motor heights above (0,0)
#define LENGTH 115                 //length of each arm-segment
...
void move_to(float X, float Y ) {
 ...
 // ----- motor step counters
  ...
  // Serial.print("current_steps2: ");
  // Serial.println(current_steps2);
  ...
  // Serial.println("Before calculate steps (x,y)");
  calculate_steps(x,y); 


  this_steps1 = STEPS1;
  this_steps2 = STEPS2;
  // Serial.print("this_steps2: ");
  // Serial.println(this_steps2);
...
void abc() {
  // ------ letter C
  pen_up();
  Serial.println("before move_to");
  move_to(71.607584, 21.035879);
  Serial.println("after move_to");
  pen_down();
  // move_to(20, 20);
  // move_to(30, 30);
  // move_to(40, 40);
